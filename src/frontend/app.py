...
st.set_page_config(page_title="Backend SDK Tester", layout="wide")
st.title("Backend SDK Tester")

# Show env status for the HyperSync token
_token = os.getenv("HYPERSYNC_API_TOKEN") or os.getenv("hypersync_api_token")
if _token:
    st.info("HyperSync API token detected in environment.")
else:
    st.warning(
