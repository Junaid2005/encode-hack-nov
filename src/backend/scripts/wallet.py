...
# Example hard-coded addresses (previous defaults)
# EXAMPLE_ADDRESSES = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect ERC20 transfers and transactions for specific wallet addresses."
    )
    parser.add_argument(
        "--addresses",
        nargs="+",
        required=True,
        help="Space-separated list of wallet addresses to monitor.",
    )
    parser.add_argument(
        "--from-block",
        type=int,
        default=0,
        help="Block number to start querying from (inclusive).",
    )
    parser.add_argument(
        "--transfer-topic",
        default=ERC20_TRANSFER_TOPIC,
        help="Topic0 hash for the ERC20 transfer event (defaults to standard Transfer topic).",
    )
...
async def main(args: argparse.Namespace):
    ...
    addresses = [address.lower() for address in args.addresses]
    address_topic_filter = list(map(address_to_topic, addresses))
    transfer_topic = args.transfer_topic.lower()
...
if __name__ == "__main__":
    # Example invocation with the previously hard-coded addresses:
    # python wallet.py --addresses \
